...
async function createConfig(path = "") {
    ...
    try { 
        ...
        await mkdir(wsfolder + "/.vscode", { recursive: true })
        const config = { saveLocation: path, include: oldconfig.include }
        ...
        console.log("Unable to create .vscode folder or write pack-config.json") }
}
/**
...
async function activate(context) {
    ...
    let dispos = vscode.commands.registerCommand('mc-pack-org.createZIP', async function () {
        ...
        config = {} 
        try { 
            config = await getConfig() } catch { console.log("Unable to read config") }
        // create zip file
        let expath = ""
        ...
        if (config.saveLocation == null) {
            expath = await getSaveLocation()
            createConfig(expath)
        } else {
            expath = config.saveLocation
        }
        ...
        for (const x in config.include) {
            ...
            try { zip.addLocalFile(wsfolder + "/" + config.include[x]) } catch { console.log("Unable to add include. Check for typos") }
        }
        zip.writeZip(expath)
        vscode.window.showInformationMessage("Zip Created")
    })
    let disposable = vscode.commands.registerCommand('mc-pack-org.packInit', async function () {
        ...
        await mkdir(wsfolder + "/data/minecraft", {recursive: true})
        await mkdir(wsfolder + "/data/" + namespace, {recursive: true})
        data = ['{\n','   "pack": {\n','      "pack_format": 36,\n','      "description": "<you pack description here>"\n','   }\n','}']
        await writeFile(wsfolder + "/pack.mcmeta", data)
        const nspath = wsfolder + "/data/" + namespace
        ...
        options.forEach (async (x) => {
            switch (x) {
                ...
                case "Functions":
                    ...
                    await mkdir(wsfolder + "/data/minecraft/tags/functions/", {recursive: true})
                    data = ['{\n','  "replace": false,\n','  "values": [\n','    "' + namespace + ':main"\n','  ]\n','}']
                    await writeFile(wsfolder + "/data/minecraft/tags/functions/" + "tick.json", data)
                    data = ['{\n','  "replace": false,\n','  "values": [\n','    "' + namespace + ':setup"\n','  ]\n','}']
                    await writeFile(wsfolder + "/data/minecraft/tags/functions/" + "load.json", data)
                    break
                case "Worldgen":
            ...
            console.log("Done!")
            vscode.window.showInformationMessage("DataPack Structure created")
        })
    })
